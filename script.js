document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.matchMedia("(pointer: fine)").matches) {
    ...
    // hover linkova, gumba i kartica
    const hoverElements = document.querySelectorAll(
      "a, button, .service-card, .work-card"
    );
...
if (contactForm) {
  contactForm.addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch("https://formspree.io/f/meedrbzw", {
        method: "POST",
        body: formData,
        headers: { Accept: "application/json" },
      });
      if (response.ok) {
        statusDiv.textContent = "Hvala! Vaša poruka je uspješno poslana.";
      ...
      statusDiv.textContent = "Mrežna greška, pokušajte kasnije.";
      statusDiv.style.color = "red";
    }
  });
}
